...
def run(args):
    ensure_dirs(args.out, args.meta)
    providers = [p.strip().lower() for p in args.providers.split(",") if p.strip()]
    queries = [q.strip() for q in args.query.split(";") if q.strip()]

    # Load existing titles to skip
    existing_titles = iter_existing_titles(args.meta)
    logging.info(f"Existing metadata titles: {len(existing_titles)}")

    # 1) Search
    papers = collect_papers(queries, providers, args.max_per_source, args.year_min, args.year_max)
    ...
    # 3) Append metadata
    if downloaded:
        append_metadata(args.meta, downloaded, file_map)
        logging.info(f"Metadata appended -> {args.meta}")

    # 4) Optionally run ingest
    if args.run_ingest:
        logging.info("Running ingest pipeline...")
        # Try several strategies so it works in most project layouts.
        launched = False
        # Strategy A: import ingest as a module and run main()
        for mod_name in ("ingest", "app.ingest", "src.ingest"):
            try:
                ...
                continue
        # Strategy B: shell out
        if not launched:
            for cmd in ("python ingest.py", "python -m app.ingest", "python -m src.ingest"):
                rc = os.system(cmd)
                if rc == 0:
                    launched = True
                    break
        if launched:
            ...
            logging.warning("Failed to run ingest automatically. Please run it manually.")
...
def build_argparser():
    ...
    ap.add_argument("--providers", default="arxiv,openalex,semanticscholar",
                    help="Comma-separated providers: arxiv,openalex,semanticscholar")
    ap.add_argument("--max-per-source", type=int, default=100, help="Max results per provider per query.")
